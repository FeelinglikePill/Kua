...
local TeamColors = {
Guards = Color3.fromRGB(0, 85, 255),       
Inmates = Color3.fromRGB(255, 140, 0),     
Criminals = Color3.fromRGB(170, 0, 0)      
}
...
local ESP_Trace = false
local TraceLines = {}

ESPTab:Toggle({
Title = "ESP Trace",
Description = "Blue line from top screen to players",
Default = false,
Callback = function(v)
ESP_Trace = v
...
if not ESP_Trace then return end
...
local function CreateItemESP(plr)
if ItemESP[plr] then return end

local gui = Instance.new("BillboardGui")  
gui.Name = "ItemESP"  
gui.Size = UDim2.new(0, 220, 0, 80)  
gui.AlwaysOnTop = true  
gui.Enabled = ItemESPEnabled  
...
data.Gui.Enabled = ItemESPEnabled  
...
data.Label.Text = table.concat(items, "\n")
...
PlayerTab:Toggle({
    Title = "Anti Ragdoll",
    Description = "Prevent ragdoll & falling",
    Default = false,
    Callback = function(v)
...
function ApplyAntiRagdoll(char)
    ...
    pcall(function()
        hum:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, not AntiRagdollEnabled)
        hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, not AntiRagdollEnabled)
        hum:SetStateEnabled(Enum.HumanoidStateType.Physics, not AntiRagdollEnabled)
    end)
    ...
    while task.wait(COOLDOWN) do
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer then
                ...
                if thrp then
                    if (hrp.Position - thrp.Position).Magnitude <= MELEE_DISTANCE then
                        pcall(function()
...
CombatTab:Toggle({
    Title = "Buff Gun",
    Description = "AutoFire / Fast Fire / High Damage",
    Default = false,
    Callback = function(state)
        buffEnabled = state
...
PlayerTab:Toggle({
    Title = "Fly",
    Description = "Move with camera direction",
    Default = false,
    Callback = function(v)
        FlyEnabled = v
        ...
        FlyConn = RunService.Heartbeat:Connect(function()
            ...
            if not LOCK_Y then
                LOCK_Y = hrp.Position.Y
            end

            local move = hum.MoveDirection
            local moveAmount = move:Dot(forward)

            if math.abs(moveAmount) > 0 then
                local newPos = hrp.Position + (forward * moveAmount * FlySpeed)
                ...
                hrp.CFrame = CFrame.lookAt(
                    Vector3.new(hrp.Position.X, LOCK_Y, hrp.Position.Z),
                    Vector3.new(hrp.Position.X, LOCK_Y, hrp.Position.Z) + forward
                )
            end
